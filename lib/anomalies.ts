...
export async function detectAnomalies(): Promise<Anomaly[]> {
  ...
  // Look at last 3 days for recent anomalies
  const since3d = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000)
  const recentTx = await prisma.transaction.findMany({
    where: { date: { gte: since3d }, isIncome: false },
    orderBy: { date: 'desc' },
  })

  if (recentTx.length === 0) return []

  // Get historical data for context (90 days)
  const since90d = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)
  const historicalTx = await prisma.transaction.findMany({
    where: { date: { gte: since90d, lt: since3d }, isIncome: false },
    select: { merchant: true, amount: true },
  })

  // Build merchant history: average amount per merchant
  ...
  for (const tx of historicalTx) {
    ...
    merchantHistory.set(tx.merchant, {
  ...
  // Check each recent transaction for anomalies
  for (const tx of recentTx) {
    const amount = Math.abs(tx.amount)
    if (amount < 100) continue // ignore tiny amounts

    const history = merchantHistory.get(tx.merchant)

    // New merchant we've never seen before with amount > 300 kr
    ...
    // Unusually large purchase — >2x the merchant average
  ...
  // Deduplicate by merchant — only report worst anomaly per merchant
  const seen = new Set<string>()
  return anomalies.filter((a) => {
    if (seen.has(a.merchant)) return false
    seen.add(a.merchant)
    return true
  })
}
