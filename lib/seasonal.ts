...
export async function storeMonthlySnapshot(year: number, month: number): Promise<void> {
...
export async function checkSeasonalMemory(): Promise<string | null> {
  const today = new Date()
  const currentMonth = today.getMonth()
  const lastYear = today.getFullYear() - 1
  const monthName = MONTH_NAMES[currentMonth]

  // Look for same month last year
  const lastYearRecord = await prisma.seasonalMemory.findUnique({
    where: { year_month: { year: lastYear, month: monthName } },
  })

  if (!lastYearRecord) return null

  // Get overall average to compare
  const allRecords = await prisma.seasonalMemory.findMany()
