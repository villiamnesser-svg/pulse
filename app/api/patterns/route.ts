...
export async function GET() {
  ...
  for (let i = 0; i < 7; i++) {
    byDay.set(i, { total: 0, count: 0, categories: new Map() })
  }

  // Count how many of each weekday exist in the period
  ...
  for (const [day, data] of byDay.entries()) {
    ...
    patterns.push({
      day,
      dayName: DAY_NAMES_SV[day],
      avgAmount: Math.round(avgAmount),
      txCount: Math.round(data.count / dayCount * 10) / 10,
      topCategory,
      isHighest: false,
      multiplierVsOthers: 0,
    })
  }

  // Mark highest and compute multiplier
  const weekdaysOnly = patterns.filter(p => p.day >= 1 && p.day <= 5)
  const maxWd = Math.max(...weekdaysOnly.map(p => p.avgAmount))
  const avgExcludingHighest = weekdaysOnly
    .filter(p => p.avgAmount < maxWd)
  ...
  for (const p of patterns) {
    ...
    p.multiplierVsOthers = avgExcludingHighest > 0
      ? Math.round((p.avgAmount / avgExcludingHighest) * 10) / 10
      : 1
  }
