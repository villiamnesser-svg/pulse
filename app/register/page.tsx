...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? 'Registrering misslyckades'); return }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 bg-[#080808]">
      <div className="w-full max-w-sm">
        <div className="bg-[#0f0f0f] border border-white/[0.08] rounded-3xl p-8">
          ...
          <h1 className="text-xl font-bold text-zinc-100 mb-1">Skapa konto</h1>
          <p className="text-sm text-zinc-500 mb-6">Gratis. Din data stannar hos dig.</p>
          ...
          <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
            ...
            >
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Skapar konto...</> : 'Skapa konto'}
            </button>
          </form>

          <p className="mt-4 text-xs text-zinc-700 text-center">
            Genom att registrera dig godkänner du vår{' '}
            <Link href="/privacy" className="text-zinc-500 hover:text-zinc-400 transition-colors underline">
              integritetspolicy
            </Link>
          </p>
        </div>
